...
// Sua vez
// const livrosDeAutorAposAno = (autor,ano) => livros.filter(livro => (livro.autor == autor && livro.anoPublicacao > ano)); // ALTERNATIVA
const livrosDeAutorAposAno = (autor,ano) => {
    return livros.filter(livro => livro.anoPublicacao > ano).filter(livro => livro.autor == autor);
}
